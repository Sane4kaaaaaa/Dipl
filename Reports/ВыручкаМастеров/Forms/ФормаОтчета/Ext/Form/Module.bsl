...
//2.1.1 Получаем данные для вывода в отчет
	...
	ВыборкаМастер = ЗапросДанных.Выполнить ().Выбрать (ОбходРезультатаЗапроса.ПоГруппировкам, "Мастер") ;
...
//2.1.2 Проводим заполнение данных в цикле по уровням группировки
	...
	//2.1.2.1 Совершаем обход результата запроса по групперовке "Мастер"
	Пока ВыборкаМастер.Следующий() Цикл
		ОбластьМастер.Параметры.Мастер = ВыборкаМастер.Мастер;
		...
		//2.1.2.2 Совершаем обход результата запроса по групперовке "Период" (внутри "Мастер")
